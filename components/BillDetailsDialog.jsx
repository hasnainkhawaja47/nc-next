...
export function BillDetailsDialog({ billId, isArchive, open, onOpenChange, fetchDetails }) {
  ...
  useEffect(() => {
    ...
    setLoading(true)
    fetchDetails(billId, isArchive)
      .then(({ bill, items }) => {
        setBill(bill)
        setItems(items)
      })
      .finally(() => setLoading(false))
  }, [open, billId, isArchive, fetchDetails])
        ...
        {loading || !bill ? (
          ...
          <div className="px-6 pb-6 space-y-4">
            ...
            <div className="rounded-xl border border-border/60 overflow-hidden">
              <div className="grid grid-cols-[1fr_56px_44px_92px_76px] gap-x-4 px-4 py-2 text-xs font-medium text-muted-foreground bg-muted/40">
                <div>Product</div>
                <div>Colour</div>
                <div>Size</div>
              ...
              <div className="divide-y divide-border/50">
                {items.map((it) => (
                  <div
                    key={it.id}
                    className="grid grid-cols-[1fr_56px_44px_92px_76px] gap-x-4 px-4 py-2 text-sm items-center"
                  >
                    ...
                    <div className="text-right tabular-nums text-muted-foreground">{it.quantity} × {it.price}</div>
